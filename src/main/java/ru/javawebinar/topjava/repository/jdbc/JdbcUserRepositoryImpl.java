...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        while (rowSet.next()) {
            map.computeIfAbsent(rowSet.getInt("user_id"), userId -> EnumSet.noneOf(Role.class));
            map.get(rowSet.getInt("user_id")).add(Role.valueOf(rowSet.getString("role")));
        }
    ...
    private void deleteRoles(User u) {
        jdbcTemplate.update("DELETE FROM user WHERE user_id=?", u.getId());
    }
    ...
    protected void batchUpdate(User user) {
        if (!CollectionUtils.isEmpty(user.getRoles())) {
            jdbcTemplate.batchUpdate("INSERT INTO user_roles(iser_id, roles) VALUES (?, ?)", new BatchPreparedStatementSetter() {
